...
def train(processed_dir: str, test_wav_dir: str, input_dir: str, output_dir: str):
    timestr = time.strftime("%Y-%m-%d-%H-%M", time.localtime())  #like '2018-10-10-14-47'
    
    all_speaker = get_speakers(input_dir)
    ...
    domain_classifier_learning_rate_decay = domain_classifier_learning_rate / 200000
    #====================load data================#
    print('Loading Data...')

    files, names = get_files_labels(os.path.join(processed_dir, '*.npy').replace('\\','/'))
    assert len(files) > 0
    ...
    for s in all_speaker:
        ...
        temp = [fn.replace('\\','/') for fn in glob.glob(p) if fn.find(s) == -1]
        exclude_dict[s] = temp
    #print (exclude_dict)
  ...
  #  model_path = os.path.join('out/100_2019-05-23-21-44/model/starganvc_model').replace('\\','/')
   # model.load('out/100_2019-05-24-15-41/model/starganvc_model')
    #====================start train==============#
    EPOCH = 1001
    ...
    for epoch in range(EPOCH):
        ...
        for i in range(num_samples // BATCHSIZE):
            ...
            for one_filename, one_name, one_target in zip(files_shuffled[start:end], names_shuffled[start:end], pre_targets):
               ...
               # target_speaker_name = target_speaker_name.split(sep='\\')[1]
                #source name
                speaker_name = one_name.split('-')[0]  #SF1
               # speaker_name = speaker_name.split(sep='\\')[1]
                #shape [36,512]
        ...
        if epoch % 10 == 0 and epoch != 0:
            print('============test model============')
            #out put path
            file_path = os.path.join(output_dir, f'{epoch}_{timestr}')
            ...
            for one_speaker in all_speaker:
                ...
                p = p.replace('\\','/')
                #wavs = glob.glob(p)
                wavs = [wav.replace('\\','/') for wav in glob.glob(p)]
            ...
            for one_file in tempfiles:
                ...
                wavpath = os.path.join(file_path, 'wavs').replace('\\','/')
        ...
        if epoch % 10 == 0 and epoch != 0:
            print('============save model============')
            model_path = os.path.join(file_path, 'model').replace('\\','/')
...
if __name__ == '__main__':
    ...
    train(processed_dir, test_wav_dir, input_dir, output_dir)
